...
func Run(files []string, stdin string) (string, string, error, string) {
	...
	stdout, stderr, err, duration := cmd.Run(workDir, "sf", "org", "login", "jwt", "--client-id", os.Getenv("CONSUMER_ID_SALESFORCE"), "--jwt-key-file", "/usr/local/bin/JWT/server.key", "--username", "daniel.borowski@coderbyte.com", "--alias", "my-hub-org", "--set-default-dev-hub")
